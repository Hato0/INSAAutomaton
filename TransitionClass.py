...
class TransitionCourbe(QGraphicsItem):
    mid_x: float
    mid_y: float
    print("TransitionCourbe Class")

    def __init__(self, state_source, state_final, name):
        super(TransitionCourbe, self).__init__()
        self.arrow_size = 7
        self.source_point = QPointF()
        self.final_point = QPointF()
        self.setAcceptedMouseButtons(Qt.NoButton)
        ...
        self.start = state_source
        self.end = state_final
        self.start.add_transition_courbe(self, name)
        self.end.add_transition_courbe(self, name)
        self.name = name
        self.adjust()
        self.selected = False
    ...
    def paint(self, painter, option, widget):
        ...
        point_inflection = self.source_point - self.final_point

        path = QPainterPath()
        path.moveTo(self.source_point)
        path.quadTo(point_inflection, self.final_point)
